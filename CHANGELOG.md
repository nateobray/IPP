...
### Added
- **PWG5100.5 Document Object** — Spec surface implementation of the IPP Document Object extension (PWG5100.5). All required attribute types, encoding, and operation payloads are implemented and unit-tested. Note: most printers including CUPS do not implement PWG5100.5 natively (CUPS uses its own `cups-get-document` instead), so these operations are spec-tested but not yet fixture-validated against real hardware.
